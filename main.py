...
class SettingsWindow(Dialog):
    ...
    def __init__(self, parent, title, keywords, time_to_record):
        self.keywords = keywords
        self.time_to_record_f = time_to_record
    ...
    def body(self, master):
        ...
        ttk.Label(master, text="Keywords:").pack(anchor='w')
        self.keyword_list = ttk.Entry(master)
        self.keyword_list.insert(0, self.keywords)
        self.keyword_list.pack()

        ttk.Label(master, text="Time to record:").pack(anchor='w')
        self.time_to_record = ttk.Entry(master)
        self.time_to_record.insert(0, self.time_to_record_f)
        self.time_to_record.pack()
    ...
    def update_settings(self):
        self.keywords = self.keyword_list.get().split(",")
        self.time_to_record = float(self.time_to_record.get())
...
class RecGui(tk.Tk):
    ...
    def __init__(self):
        ...
        self.time_to_record = 5.0

        self.transcibed_window = tk.Text(self, height=10, width=50, wrap=tk.WORD)
        self.transcibed_window.pack()
        access_key = "b+KQFVbbwp4BkhKkXsH1WJJEvGgZ8FygX5CRAL7/aBRQWzJZSFFHWw=="
        keywords = ["porcupine"]
    ...
    def create_stream(self, device=None):
        ...
        self.stream = sd.InputStream(
            device=device, channels=1, dtype='int16', callback=self.audio_callback)
        self.stream.start()
    ...
    def audio_callback(self, indata, frames, time, status):
        ...
        if self.recording:
            ...
            if self.previously_recording:
                ...
                results = self.porcupine.process(reduce(lambda a,b:a+b, indata.copy().tolist()))
                if results >= 0:
                    self.keyword_q.put(self.keywords[results])
                    print('[%s] Detected %s' % (results, self.keywords[results]))
                    if self.keywords[results] == "porcupine":
                        timer = threading.Timer(5.0, lambda: self.on_stop())
                        timer.start()
                        self.on_rec()
    ...
    def transcribe_thread(self, filename):
        result = self.model.transcribe(filename)
        self.transcribe_file = result
    ...
    def on_settings(self, *args):
        w = SettingsWindow(self, 'Settings', self.keywords, self.time_to_record)
        if w.result is not None:
            self.create_stream(device=w.result)
        if w.keywords is not None:
            self.keywords = w.keywords
        if w.time_to_record is not None:
            self.time_to_record = w.time_to_record
